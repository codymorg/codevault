/******************************************************************************
  * Project Euler Solutions
  * Author : Cody Morgan
  * Brief  : This is a collection of my solutions to ProjectEuler
  *          and Leetcode problems mostly done for fun
  *          This was orginally done in C# 2013 while I was deployed with the
  *          Army but I was a young and reckless programmer so now I'm doing
  *          it the right way - or at least a better way

******************************************************************************/

#include "StandardIncludes.h"
#include "ProjectEulerProblems.h"
#include "Utility.h"
#include "MathUtility.h"



// actual problems
namespace ProjectEulerProblems
{
  void Euler1()
  {
    cout << "what is the sum of all factors of 3 or 5 < 1000?\n";

    int sum = 0;
    for (int i = 3; i < 1000; i++)
    {
      if ((i % 3 == 0) || (i % 5) == 0)
      {
        sum += i;
      }
    }

    cout << "answer: " << sum << "\n";
  }

  void Euler2()

  {
    cout << "what is the sum of all even fibonacci numbers less than 4000000?\n";

    unsigned num = 1;
    unsigned back1 = 2;
    unsigned back2 = 1;
    unsigned sum = 0;

    // calculate fibonacci values
    while (num < 4000000)
    {
      num = back1 + back2;
      back2 = back1;
      back1 = num;

      // sum only those that are even
      if (num % 2 == 0)
        sum += num;
    }

    cout << "answer: " << sum << "\n";
  }

  void Euler3()
  {
    cout << "what is the largest prime factor of 600851475143?\n";

    long long number = 600851475143;
    long largestFactor = long(sqrt(number));         // there's no factor greater than the sqrt of a number
    vector<long> primes = PrimeSieve(largestFactor); // get all primes that might be factors

    // check each prime to see if it's a factor
    for (long i = long(primes.size()) - 1; i >= 0; i--)
    {
      if (number % primes[i] == 0)
      {
        largestFactor = primes[i];
        break;
      }
    }

    cout << "answer: " << largestFactor << "\n";
  }

  void Euler4()
  {
    cout << "Find the largest palindrome made from the product of two 3-digit numbers.\n";

    char palindrome[7] = {}; // no 3 digit multiplication is greater than 6 digits

    for (unsigned factor1 = 999; factor1 >= 100; factor1--)
    {
      for (unsigned factor2 = 999; factor2 >= 100; factor2--)
      {
        // convert number into char array for palindrome testing
        unsigned testFactor = factor1 * factor2;
        _itoa_s(testFactor, &palindrome[0], _countof(palindrome), 10);

        // if it's a plindrome we're done here
        if (StringManipulation::IsPalindrome(palindrome, _countof(palindrome)))
        {
          // let's assume that it starts with a 9 and is 6 digits
          if (palindrome[0] == '9')
          {
            cout << "answer: " << palindrome << "\n";
            return;
          }
        }
      }
    }
  }

  void Euler5()
  {
    cout << "What is the smallest positive number that is evenly divisible by all of the numbers from 1 to 20?\n";

    unsigned x = 20;
    unsigned num = x;
    unsigned factor = x;
    bool done = false;

    while (done == false)
    {
      if (num % factor == 0 && factor > 0)
      {
        factor--;
      }
      else
      {
        factor = x;
        num += x;
      }
      if (factor == 0)
      {
        done = true;
      }
    }
    
    cout << "answer: " << num << '\n';
  }
  
  void Euler6()
  {
    cout << "Find the difference between the sum of the squares of the first one hundred natural numbers and the square of the sum.\n";

    //square of sum - sum of square
    long square;
    long sum = 0;
    long num = 1;
    while (num < 101)
    {
      square = num * num;
      sum = sum + square;
      num++;
    }

    num = 1;
    square = 0;
    while (num < 101)
    {
      square += num;
      num++;
    }
    square *= square;
    num = square - sum;
    cout << "Answer: " << num << "\n";
  }

  void Euler7()
  {
    cout << "What is the 10001st prime number?\n";

    // we're guarenteed to find X primes under 90X
    vector<unsigned>primes = PrimeSieve(unsigned(10001 * 90));

    cout << "answer: " << primes[10001] << "\n";
  }

  void Euler8()
  {
    {
      cout << "73167176531330624919225119674426574742355349194934\n";
      cout << "96983520312774506326239578318016984801869478851843\n";
      cout << "85861560789112949495459501737958331952853208805511\n";
      cout << "12540698747158523863050715693290963295227443043557\n";
      cout << "66896648950445244523161731856403098711121722383113\n";
      cout << "62229893423380308135336276614282806444486645238749\n";
      cout << "30358907296290491560440772390713810515859307960866\n";
      cout << "70172427121883998797908792274921901699720888093776\n";
      cout << "65727333001053367881220235421809751254540594752243\n";
      cout << "52584907711670556013604839586446706324415722155397\n";
      cout << "53697817977846174064955149290862569321978468622482\n";
      cout << "83972241375657056057490261407972968652414535100474\n";
      cout << "82166370484403199890008895243450658541227588666881\n";
      cout << "16427171479924442928230863465674813919123162824586\n";
      cout << "17866458359124566529476545682848912883142607690042\n";
      cout << "24219022671055626321111109370544217506941658960408\n";
      cout << "07198403850962455444362981230987879927244284909188\n";
      cout << "84580156166097919133875499200524063689912560717606\n";
      cout << "05886116467109405077541002256983155200055935729725\n";
      cout << "71636269561882670428252483600823257530420752963450\n";
    }
    cout << "Find the thirteen adjacent digits in the 1000-digit number that have the greatest product. What is the value of this product?\n";

    string huge = "";
    unsigned long long max = 0;           // what max we've found
    constexpr unsigned productCount = 13; // how many numbers are in the product

    // check product values
    for (unsigned start = 0; start < huge.size(); start++)
    {
      // get product of next 13 values
      unsigned long long current = 1;
      for (unsigned i = 0; i < productCount; i++)
      {
        unsigned end = start + i;
        unsigned currentChar = huge[end] - '0';
        current *= currentChar;

        // we hit a 0 there's no way it's the max
        if (current == 0)
        {
          start += i;
          break;
        }
      }

      // update max
      if (current > max)
      {
        max = current;
      }
    }

    cout << "answer: " << max << "\n";
  }

  void Euler9()
  {
    cout << "There exists exactly one Pythagorean triplet for which a + b + c = 1000. Find the product abc.\n";

    unsigned a = 1;
    unsigned b = 2;
    unsigned c = 997;
    unsigned count = 0;

    while (b > a)
    {
      while (c > b)
      {
        // pythag check!
        if ((c * c) == ((a * a) + (b * b)))
        {
          cout << "answer: " << a * b * c << "\n";
          return;
        }

        // not pythagorian - keep checking
        else
        {
          
          b++;
          c--;
          count++;
        }
      }

      // update values
      a++;
      b += 1 - count;
      c += count - 2;
      count = 0;
    }
  }

  void Euler10()
  {
    cout << "Find the sum of all the primes below two million.\n";

    vector<unsigned long long>primes = PrimeSieve(unsigned long long(2000000));
    unsigned long long sum = 0;
    for (unsigned long long i : primes)
    {
      sum += i;
    }

    cout << "answer: " << sum << "\n";
  }

  void Euler11()
  {
    int limit = 20, mult = 0, max = 0, count = 0;
    int matrix[] = {   8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8,
                       49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48, 4, 56, 62, 0,
                       81, 49, 31, 73, 55, 79, 14, 29, 93, 71, 40, 67, 53, 88, 30, 03, 49, 13, 36, 65,
                       52, 70, 95, 23, 04, 60, 11, 42, 69, 24, 68, 56, 01, 32, 56, 71, 37, 02, 36, 91,
                       22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 40, 40, 28, 66, 33, 13, 80,
                       24, 47, 32, 60, 99, 03, 45, 02, 44, 75, 33, 53, 78, 36, 84, 20, 35, 17, 12, 50,
                       32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18, 38, 64, 70,
                       67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 8, 40, 91, 66, 49, 94, 21,
                       24, 55, 58, 05, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72,
                       21, 36, 23, 9, 75, 00, 76, 44, 20, 45, 35, 14, 00, 61, 33, 97, 34, 31, 33, 95,
                       78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 03, 80, 04, 62, 16, 14, 9, 53, 56, 92,
                       16, 39, 05, 42, 96, 35, 31, 47, 55, 58, 88, 24, 00, 17, 54, 24, 36, 29, 85, 57,
                       86, 56, 00, 48, 35, 71, 89, 07, 05, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58,
                       19, 80, 81, 68, 05, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 04, 89, 55, 40,
                       04, 52, 8, 83, 97, 35, 99, 16, 07, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66,
                       88, 36, 68, 87, 57, 62, 20, 72, 03, 46, 33, 67, 46, 55, 12, 32, 63, 93, 53, 69,
                       04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32, 40, 62, 76, 36,
                       20, 69, 36, 41, 72, 30, 23, 88, 34, 62, 99, 69, 82, 67, 59, 85, 74, 04, 36, 16,
                       20, 73, 35, 29, 78, 31, 90, 01, 74, 31, 49, 71, 48, 86, 81, 16, 23, 57, 05, 54,
                       01, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 01, 89, 19, 67, 48 };
    while (limit <= _countof(matrix))
    {
      for (int i = 0; count + i <= limit - 4; i++)//horiz
      {
        mult = matrix[count + i] * matrix[count + i + 1] * matrix[count + i + 2] * matrix[count + i + 3];
        if (mult > max)
          max = mult;
        if (i + 3 < 20 && count < 337) //diag left
        {
          mult = matrix[count + i] * matrix[count + i + 21] * matrix[count + i + 42] * matrix[count + i + 63];
          if (mult > max)
            max = mult;
        }
        if (i >= 3 && count < 340)//diag right
        {
          mult = matrix[count + i] * matrix[count + i + 19] * matrix[count + i + 38] * matrix[count + i + 57];
          if (mult > max)
            max = mult;
          if ((count + i) == (limit - 4))
          {
            for (int j = 1; j < 4; j++) //diag right last three rows
            {
              mult = matrix[count + i + j] * matrix[count + i + 19 + j] * matrix[count + i + 38 + j] * matrix[count + i + 57 + j];
              if (mult > max)
                max = mult;
            }
          }
        }
      }
      for (int i = 0; i < 340; i++)//vert
      {
        mult = matrix[i] * matrix[i + 20] * matrix[i + 40] * matrix[i + 60];
        if (mult > max)
          max = mult;
      }
      limit = limit + 20;
      count = count + 20;
    }
    limit = 20;

    cout << "answer: " << max << "\n";
  }

  void Euler12()
  {
    cout << "1 : 1\n";
    cout << "3 : 1, 3\n";
    cout << "6 : 1, 2, 3, 6\n";
    cout << "10 : 1, 2, 5, 10\n";
    cout << "15 : 1, 3, 5, 15\n";
    cout << "21 : 1, 3, 7, 21\n";
    cout << "28 : 1, 2, 4, 7, 14, 28\n";
    cout << "We can see that 28 is the first triangle number to have over five divisors.\n";
    cout << "What is the value of the first triangle number to have over five hundred divisors ?\n";


    unsigned triangle = 1;             // natural triangle
    unsigned tri;                      // for modification
    unsigned naturalNum = 1;           // next natural number
    unsigned count = 0;                // factor count
    unsigned exponent;                 // prime expoonent
    const unsigned primeCount = 65000; // arbitrary number of primes - it's probably enough

    vector<unsigned> primes = PrimeSieve(primeCount);

    while (count <= 500)
    {
      // set up next triangle number
      count = 1;
      naturalNum++;
      triangle += naturalNum;
      tri = triangle;

      
      for (unsigned i = 0; i<primeCount; i++) 
      {
        if (primes[i] * primes[i] > tri)
        {
          count *= 2;
          break;
        }
        exponent = 1;
        while (tri % primes[i] == 0)
        {
          exponent++;
          tri /= primes[i];
        }
        if (exponent > 1)
          count *= exponent;
        if( tri == 1)
          break;
        }
    }

    cout << "answer: " << triangle << "\n";
  }

  void Euler13()
  {
    string hugeArray[] = 
    { 
      "37107287533902102798797998220837590246510135740250",
      "46376937677490009712648124896970078050417018260538",
      "74324986199524741059474233309513058123726617309629",
      "91942213363574161572522430563301811072406154908250",
      "23067588207539346171171980310421047513778063246676",
      "89261670696623633820136378418383684178734361726757",
      "28112879812849979408065481931592621691275889832738",
      "44274228917432520321923589422876796487670272189318",
      "47451445736001306439091167216856844588711603153276",
      "70386486105843025439939619828917593665686757934951",
      "62176457141856560629502157223196586755079324193331",
      "64906352462741904929101432445813822663347944758178",
      "92575867718337217661963751590579239728245598838407",
      "58203565325359399008402633568948830189458628227828",
      "80181199384826282014278194139940567587151170094390",
      "35398664372827112653829987240784473053190104293586",
      "86515506006295864861532075273371959191420517255829",
      "71693888707715466499115593487603532921714970056938",
      "54370070576826684624621495650076471787294438377604",
      "53282654108756828443191190634694037855217779295145",
      "36123272525000296071075082563815656710885258350721",
      "45876576172410976447339110607218265236877223636045",
      "17423706905851860660448207621209813287860733969412",
      "81142660418086830619328460811191061556940512689692",
      "51934325451728388641918047049293215058642563049483",
      "62467221648435076201727918039944693004732956340691",
      "15732444386908125794514089057706229429197107928209",
      "55037687525678773091862540744969844508330393682126",
      "18336384825330154686196124348767681297534375946515",
      "80386287592878490201521685554828717201219257766954",
      "78182833757993103614740356856449095527097864797581",
      "16726320100436897842553539920931837441497806860984",
      "48403098129077791799088218795327364475675590848030",
      "87086987551392711854517078544161852424320693150332",
      "59959406895756536782107074926966537676326235447210",
      "69793950679652694742597709739166693763042633987085",
      "41052684708299085211399427365734116182760315001271",
      "65378607361501080857009149939512557028198746004375",
      "35829035317434717326932123578154982629742552737307",
      "94953759765105305946966067683156574377167401875275",
      "88902802571733229619176668713819931811048770190271",
      "25267680276078003013678680992525463401061632866526",
      "36270218540497705585629946580636237993140746255962",
      "24074486908231174977792365466257246923322810917141",
      "91430288197103288597806669760892938638285025333403",
      "34413065578016127815921815005561868836468420090470",
      "23053081172816430487623791969842487255036638784583",
      "11487696932154902810424020138335124462181441773470",
      "63783299490636259666498587618221225225512486764533",
      "67720186971698544312419572409913959008952310058822",
      "95548255300263520781532296796249481641953868218774",
      "76085327132285723110424803456124867697064507995236",
      "37774242535411291684276865538926205024910326572967",
      "23701913275725675285653248258265463092207058596522",
      "29798860272258331913126375147341994889534765745501",
      "18495701454879288984856827726077713721403798879715",
      "38298203783031473527721580348144513491373226651381",
      "34829543829199918180278916522431027392251122869539",
      "40957953066405232632538044100059654939159879593635",
      "29746152185502371307642255121183693803580388584903",
      "41698116222072977186158236678424689157993532961922",
      "62467957194401269043877107275048102390895523597457",
      "23189706772547915061505504953922979530901129967519",
      "86188088225875314529584099251203829009407770775672",
      "11306739708304724483816533873502340845647058077308",
      "82959174767140363198008187129011875491310547126581",
      "97623331044818386269515456334926366572897563400500",
      "42846280183517070527831839425882145521227251250327",
      "55121603546981200581762165212827652751691296897789",
      "32238195734329339946437501907836945765883352399886",
      "75506164965184775180738168837861091527357929701337",
      "62177842752192623401942399639168044983993173312731",
      "32924185707147349566916674687634660915035914677504",
      "99518671430235219628894890102423325116913619626622",
      "73267460800591547471830798392868535206946944540724",
      "76841822524674417161514036427982273348055556214818",
      "97142617910342598647204516893989422179826088076852",
      "87783646182799346313767754307809363333018982642090",
      "10848802521674670883215120185883543223812876952786",
      "71329612474782464538636993009049310363619763878039",
      "62184073572399794223406235393808339651327408011116",
      "66627891981488087797941876876144230030984490851411",
      "60661826293682836764744779239180335110989069790714",
      "85786944089552990653640447425576083659976645795096",
      "66024396409905389607120198219976047599490197230297",
      "64913982680032973156037120041377903785566085089252",
      "16730939319872750275468906903707539413042652315011",
      "94809377245048795150954100921645863754710598436791",
      "78639167021187492431995700641917969777599028300699",
      "15368713711936614952811305876380278410754449733078",
      "40789923115535562561142322423255033685442488917353",
      "44889911501440648020369068063960672322193204149535",
      "41503128880339536053299340368006977710650566631954",
      "81234880673210146739058568557934581403627822703280",
      "82616570773948327592232845941706525094512325230608",
      "22918802058777319719839450180888072429661980811197",
      "77158542502016545090413245809786882778948721859617",
      "72107838435069186155435662884062257473692284509516",
      "20849603980134001723930671666823555245252804609722",
      "53503534226472524250874054075591789781264330331690" 
    };

    cout << "caclulate the first 10 digits of the sum of these numbers:\n";

    Huge sum;

    for (string str : hugeArray)
    {
      cout << str << "\n";
      sum.addString(str);
    }
      cout << "answer: " << sum << '\n';
  }

  void Euler14()
  {
    cout << "The following iterative sequence is defined for the set of positive integers:\n";
    cout << "n -> n / 2 (n is even)\n";
    cout << "n -> 3n + 1 (n is odd)\n";
    cout << "Using the rule above and starting with 13, we generate the following sequence :\n";
    cout << "13 -> 40 -> 20 -> 10 -> 5 -> 16 -> 8 -> 4 -> 2 -> 1\n";
    cout << "It can be seen that this sequence(starting at 13 and finishing at 1) contains 10 terms.\nAlthough it has not been proved yet(Collatz Problem), it is thought that all starting numbers finish at 1.\n";
    cout << "Which starting number, under one million, produces the longest chain ?\n\n";

    constexpr unsigned guarenteedMax = 1000001;
    unsigned* cache = new unsigned[guarenteedMax];
    memset(cache, 0, sizeof(unsigned) * guarenteedMax);
    unsigned count = 0; 
    unsigned countMax = 0;
    unsigned answer = 0;

    for (unsigned startingPoint = 4; startingPoint < guarenteedMax; startingPoint++)
    {
      unsigned currentSequenceValue = startingPoint;
      count = 0;

      while (currentSequenceValue > 1)
      {
        count++;

        // iterative sequence defined as n/2 for even and 3n+1 if odd
        if (currentSequenceValue % 2 == 0)
          currentSequenceValue /= 2;
        else
          currentSequenceValue = (currentSequenceValue * 3) + 1;

        // this number's sequence value has already been calculated - use it instead of redoing it
        if (currentSequenceValue < guarenteedMax && cache[currentSequenceValue] > 0)
        {
          count += cache[currentSequenceValue];
          currentSequenceValue = 1;
        }
      }

      // keep track of the highpoints
      if (count > countMax)
      {
        countMax = count;
        answer = startingPoint;
      }

      // if we ever come across this number we can use its cache value instead of recalculating the whole sequence
      if (cache[startingPoint] == 0)
      {
        cache[startingPoint] = count;
      }
    }

    cout << "answer: " << answer << "\n";
  }

  void Euler15()
  {
    cout << "How many efficient routes are there through a 20 X 20 grid?\n";

    // this can be solved be using n choose k, where n is width + hight and k is the smallest of the two
    // choose(n,k) -> combinations where order does not matter
    cout << "answer: " << nChooseK(40, 20) << "\n";
  }

  void Euler16()
  {
    cout << "what is the sum of the digits in the number 2^1000?\n";
    
    Huge product(1);

    // find the power
    for (unsigned i = 0; i < 1000; i++)
    {
      product *= 2;
    }

    unsigned long sum = 0;
    std::list<unsigned char>::const_iterator current = product.begin();
    while (current != product.end())
    {
      sum += (*current);
      current++;
    }

    cout << "answer: " << sum << "\n";
  }

  void Euler17()
  {
    cout << "If the numbers 1 to 5 are written out in words : one, two, three, four, five, then there are 3 + 3 + 5 + 4 + 4 = 19\nletters used in total.\n";
    cout << "If all the numbers from 1 to 1000 (one thousand) inclusive were written out in words, how many letters would be used ?\n\n";

    // list of words, the first spot is left blank so it can be summed without affecting the value
    string thousand = "oneThousand";
    string hundred = "hundred";
    string british = "and";
    string tens[] = { "", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety" };
    string teens[] = {"ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", "seventeen", "eighteen", "nineteen" };
    string ones[] = {"", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine" };
    unsigned sum = 0;
    unsigned count = 0;

    // run through all the word combos
    for (unsigned hundreds = 0; hundreds < 10; hundreds++)
    {
      for (unsigned ten = 0; ten < 10; ten++)
      {
        for (unsigned one = 0; one < 10; one++)
        {
          // handle hundreds
          if (hundreds > 0)
          {
            sum += unsigned(ones[hundreds].size() + hundred.size());
            if (ten > 0 || one > 0)
            {
              sum += unsigned(british.size());
            }
          }

          // ten's place and one's place
          if (ten == 1)
          {
            sum += unsigned(teens[one].size());
          }
          else if(ten > 1)
          {
            sum += unsigned(tens[ten].size() + ones[one].size());
          }
          else if (ten == 0)
          {
            sum += unsigned(ones[one].size());
          }

          count++;
        }
      }
    }
    sum += unsigned(thousand.size());

    cout << "answer: " << sum << "\n";
  }
}


// project management
namespace ProjectEulerProblems
{
  typedef void(*Euler)();
  std::vector<Euler> solutions =
  {
    Euler1,  Euler2,  Euler3,  Euler4,  Euler5,  Euler6,  Euler7,  Euler8,  Euler9,  Euler10, 
    Euler11, Euler12, Euler13, Euler14, Euler15, Euler16, Euler17,
  };

  void RunEulerSolution(unsigned number)
  {
    if (number <= solutions.size())
    {
      StopWatch sw;
      sw.start();

      try
      {
        solutions[number - 1]();
      }
      catch (std::exception & e)
      {
        cout << e.what() << "\n";
      }

      sw.end();
      sw.printElapsed();
    }
    else
    {
      cout << "ERROR There is no Euler " << number << " solution\n";
    }
  }

  unsigned GetTotalSolved()
  {
    return unsigned(solutions.size());
  }
}

namespace HelperFunctions
{

}